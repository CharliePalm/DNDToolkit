...
from shared.notion.notion import (
    Notion,
    get_block_type_val,
    get_page_title,
    pprint,
...
class TemplateFiller:
    ...
    all_weapons: List[Weapon] = (
        []
    )  # load_json_object('./misc_loader/weapons.json', Weapon)
    ...
    def is_proficient_in_weapon(self, weapon: Weapon):
        return weapon.family + " weapons" in self.char.proficiencies["Weapons"]
    ...
    def create_weapon_block(self, to_send, weapon: Weapon):
        ...
        update_page_value(
            to_send,
            (
                f"{weapon.range_max} max, {weapon.range_normal}, normal"
    ...
    def title(self):
        ...
        self.notion.client.pages.update(
            self.template_id, properties=self.page["properties"]
        )

    def header_blocks(self):
        hdr_block = self.notion.get_block_children(self.top_level_children[3]["id"])
        text_to_update = [
            self.char.character_class,
            self.char.species,
            self.char.background or "",
            char.alignment or "",
            "",
            char.level,
        ]
        for i in range(3):
            ...
            for j in range(2):
                ...
                self.notion.client.blocks.update(
                    children[j]["id"],
                    **{children[j]["type"]: get_block_type_val(children[j])},
                )

    def left_stat_col(self):
        skills_db_id = self.notion.get_block_children(self.stats_block[0]["id"])[0][
            "id"
        ]
        ...
        for skill_page in skills:
            ...
            char_skill_val = (
                self.char.skills[name]
                if name != "Proficiency Bonus"
                else self.char.proficiency_bonus
            )
            if name == "Strength":
    ...
    def middle_stats_col(self):
        ...
        check_entries: List[dict] = self.notion.client.databases.query(
            middle_col[1]["id"]
        )["results"]
    ...
    def right_stats_col(self):
        right_col = filter(
            lambda x: x["type"] == "callout",
            self.notion.get_block_children(self.stats_block[2]["id"]),
        )
        data = [
            (self.char.max_hp, self.char.current_hp),
            self.char.ac,
            f"{self.char.level}d{self.char.hit_die}",
            "SKIP",
            ("+" if self.char.skills[Skill.Dexterity.value] > 10 else "")
            + str(self.char.get_modifier(Skill.Dexterity.value)),
            "SKIP",
            self.char.get_spell_casting_mod(),
        ]
        for block, to_update in zip(right_col, data):
            ...
            self.notion.client.blocks.update(
                block["id"], **{block["type"]: get_block_type_val(block)}
            )

    def features_dbs(self):
        for idx, char_features in [
            (6, self.char.features),
            (7, self.char.other_features),
        ]:
            if char_features:
                ...
                block_template = self.notion.scrape_result(
                    deepcopy(self.notion.get_block_children(default_entry["id"])[0])
                )
                ...
                for feature in char_features:
                    ...
                    new_page = self.notion.create_db_page(
                        class_features_db_id, feature, "✨", to_send["properties"]
                    )
                    self.notion.append_block(new_page["id"], [block_to_send])

    def proficiencies_db(self):
        block_template = self.notion.scrape_result(
            self.notion.query_db(self.profs_and_weapons[0]["id"])[0]
        )
        for prof in self.char.proficiencies:
            if type(self.char.proficiencies[prof]) == list:
                for p in self.char.proficiencies[prof]:
                    ...
                    self.notion.create_db_page(
                        self.profs_and_weapons[0]["id"], p, "➕", to_send["properties"]
                    )
            else:
                to_send = deepcopy(block_template)
                update_page_value(to_send, p, "Name")
    ...
    def weapons_db(self):
        profs_and_weapons = self.notion.get_block_children(
            self.inventory_blocks[0]["id"]
        )
        block_template = self.notion.scrape_result(
            self.notion.query_db(profs_and_weapons[1]["id"])[0]
        )
        for weapon in self.get_equipment(True):
            to_send = deepcopy(block_template)
            self.create_weapon_block(to_send, weapon)
            self.notion.create_db_page(
                profs_and_weapons[1]["id"],
                weapon.name,
                get_weapon_emoji(weapon),
                to_send["properties"],
            )

    def money_and_equipment(self):
        money_and_equipment = self.notion.get_block_children(
            self.inventory_blocks[1]["id"]
        )
        block_template = self.notion.scrape_result(
            self.notion.query_db(money_and_equipment[2]["id"])[0]
        )
        for eq_name, eq_type in self.get_equipment(False):
            ...
            self.notion.create_db_page(
                money_and_equipment[2]["id"], eq_name, "🎒", to_send["properties"]
            )
...
def fill_character_sheet_template(template_id: str, character_name: str):
    ...
    """
    template_filler.stats_block = template_filler.notion.get_block_children(
        template_filler.top_level_children[4]["id"]
    )
    ...
    template_filler.inventory_blocks = template_filler.notion.get_block_children(
        template_filler.top_level_children[9]["id"]
    )
