...
class Notion:
    ...
    def scrape_result(self, result, remove_ids=False):
        ...
        for v in [
            ...
            "public_url",
            "request_id",
            "id" if remove_ids else None,
            "parent" if remove_ids else None,
        ]:
            if v in result:
                del result[v]
        for property in result:
            if (
                type(result[property]) == dict or type(result[property]) == list
            ) and property != "mention":
    ...
    def create_weapon(self, weapon: Weapon):
        return self.client.pages.create(
            ...
            icon={
                "emoji": (
                    "🗡️"
                    if weapon.range_normal == 0 or "Ammunition" not in weapon.properties
                    else "🏹"
                )
            },
                ...
                children=[
                    {
                        ...
                        p_type: {
                            "rich_text": [
                                {"type": "text", "text": {"content": desc_paragraph}}
                            ]
                        },
                    }
                ],
    ...
    def upload_character(self, character_name: str):
        ...
        add side blocks
        """
        character: Character = load_json_object(
            f"./generator/{character_name}.json", Character
        )
        self.deep_copy_page(
            "b00ddeb5e9e944269066508dce078d26",
            "page_id",
            "2e7458dcad594ba2ad2fc57d63c75fdd",
        )
    ...
    def _create_db_test(self):
        db = self.scrape_result(
            load_json_object("./artifacts/notion/DB.json", dict), True
        )
        db["parent"] = {
            "type": "page_id",
    ...
    def append_block(self, parent_id, children):
        with open("./body.json", "w") as fp:
            json.dump(children, fp, indent=4)
        return (
            self.client.blocks.children.append(parent_id, children=children)
            if len(children)
            else []
        )
