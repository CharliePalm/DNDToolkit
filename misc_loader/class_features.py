...
def _clean_text(node: Tag) -> str:
    ...
    return re.sub(r'\s+', ' ', node.get_text().replace(u'\xa0', u' ')).strip()
...
def _parse_level_table(table: Tag) -> Tuple[Dict[str, int], Dict[str, dict]]:
    ...
    resource_cols = {i: header_cells[i] for i in range(len(header_cells)) if i > features_col}

    feature_levels: Dict[str, int] = {}
    resource_values: Dict[str, Dict[int, object]] = {name: {} for name in resource_cols.values()}
    ...
    for col_name, per_level in resource_values.items():
        singular = _normalize_name(col_name[:-1] if col_name.endswith('s') else col_name)
...
def _parse_base_class(html: str, char_class: str) -> Tuple[List[ClassFeature], List[str]]:
...
def _find_subclass_keys(content: Tag, class_page_key: str) -> List[str]:
    ...
    seen = set()
    pattern = re.compile(
        r'^(?:https?://dnd5e\.wikidot\.com)?/(' + re.escape(class_page_key) + r':[a-z0-9-]+)$'
    )
...
def get_class_features(char_class: str, is_dry_run: bool = False, save: bool = True) -> List[ClassFeature]:
    '''
    ...
    if save:
        write_obj_to_json(all_features, os.path.join(OUTPUT_DIR, page_key + '_features.json'))
    return all_features
