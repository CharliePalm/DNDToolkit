...
def get_all_weapons(is_dry_run):
    ...
    for idx, table in enumerate(equipment_tables):
        ...
        for row in rows:
            tds = [r.get_text().replace("\xa0", " ") for r in row.find_all("td")]
            if tds:
                weapon = Weapon()
                weapon.name = tds[0]
                ...
                if tds[1].split(" ")[1] != "gp":
                    if tds[1].split(" ")[1] == "sp":
                        weapon.cost = int(weapon.cost / 10)
                    else:
                        weapon.cost = int(weapon.cost / 100)
                if tds[2] != "—":
                    weapon.damage, weapon.damage_type = tds[2].split(" ")
                    weapon.damage_type = weapon.damage_type.capitalize()
                else:
                    weapon.damage, weapon.damage_type = 0, ""
                ...
                for p in weapon.properties:
                    ...
                    if "thrown" in lower_p or "range" in lower_p:
                        to_remove = p
                        weapon.range_normal, weapon.range_max = [
                            int(v)
                            for v in re.search(r"\((.*?)\)", p).group(1).split("/")
                        ]
                    if p == "—":
                        to_remove = p
                if to_remove:
                ...
                weapon.family = (
                    WeaponFamily.Simple.value
                    if idx == 0
                    else WeaponFamily.Martial.value
                )
...
def get_all_backgrounds(is_dry_run):
    ...
    bgs: List[Tag] = [
        (
            row.find("td", attrs={"class": "column-1"}),
            row.find("td", attrs={"class": "column-6"}),
        )
        for row in rows[1:]
    ]
    ...
    for key, val in bgs[1:]:
        if not val or len(val.text) == 0 or "special" in val.text:
            continue
        val = val.text
        if "choose" in val.lower():
            val = re.sub(
                r"\bChoose\s+(\d+):", r"\1" + ",", val, flags=re.IGNORECASE
            ).replace(", and ", ", ")
        val = [
            word.strip().capitalize()
            for word in re.sub(r"\(.*?\)", "", val.replace(" and ", ", ")).split(", ")
        ]
