...
def add_description_info(spell: Spell):
    spell.saving_throw, spell.damage, spell.damage_type = parse_damage_info(
        spell.description
    )
    try:
        spell.saving_throw = (
            Skill[spell.saving_throw].value
            if spell.saving_throw and spell.saving_throw
            else ""
        )
    except:
        spell.saving_throw = ""
    ...
    if "d" not in spell.damage and spell.damage != "":
        spell.damage = ""
    spell.damage_type = (
        DamageType[spell.damage_type].value
        if spell.damage_type and spell.damage_type in DamageType.__dict__
        else ""
    )
